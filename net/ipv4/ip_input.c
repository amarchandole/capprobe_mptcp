...
void process_capprobe(struct sk_buff *skb, struct net_device *dev, const struct iphdr *iph)
{
    ...
    { 	
        ...
        //printk (KERN_INFO "%ld %ld ICMP RECV ID_%d serialnum= %d  (sec,usec)= %ld %ld\n",cap_size, cap_id, id,tmpvar,tv.tv_sec, tv.tv_usec);
        ...
        {
        	//printk(KERN_INFO "CapProbe packet received by processCapprobe\n");
        	for (j = 0; j < MAX_BURST_SIZE; j++) 
        	{
        		//printk(KERN_INFO "CapProbe packet serial number: temp value: %u, serial 1st: %u, serial 2nd: %u", tmpvar, cap_serialnum[j], cap_serialnum[j] + 1);
                ...
                { 
                	...
                	printk(KERN_INFO "CapProbe first packet received\n");
                	break;
                } 
                ...
                {	
                	...
                	printk(KERN_INFO "CapProbe second packet received\n");
                	if (cap_recv1_sec[j]>0 && cap_recv1_usec[j]>0) 
                	{
                		...
                		if (disp>0 && rtt1>0 && rtt2>0 && rtt>0 && rtt<TOO_LARGE_DELAY) 
                		{
                            ...
                            cap_recv_num++;
                            printk(KERN_INFO "Getting minimum delay value RTT1: %ld, RTT2: %ld, cap_receive number: %ld", cap_RTT1, cap_RTT2, cap_recv_num);
                    ...
                    {
                        // disorder, ignore it!
                    	printk(KERN_INFO "Packet pair sent in disorder, ignore it");
                    }
                    break;
                } 
                else 
                {
                	printk(KERN_INFO "serialnum not matched\n");
                	continue;
                }
            }
            ...
            if (cap_C_same2 >= CAP_SAME_MAX || (cap_recv_num >= CAP_SAMPLES && j < MAX_BURST_SIZE && cap_C_same >= CAP_SAME) ) 
            {
            	...
            	printk(KERN_INFO "further calculate difference of RTT\n");
                ...
                if (cap_C_same2>=CAP_SAME_MAX||diff_cap_RTT_SUM < 10) 			//previously 0.01, changed to 10 because ---> diff_cap_RTT_SUM *= 1000;			//to maintain precision
                {
                    ...
                    //cs218 large printk section removed, verify if needed later
                	printk(KERN_INFO "Algorithm converged check\n");
                	...
                	if (cap_C_same2 >= CAP_SAME_MAX) 
                	{
                		del_timer(&tl);
                		cap_size = 0;
                		printk(KERN_INFO "Stable CapProbe Finished! Total time = %ld.%06ld sec C = %ld.%03ld\n",
                	...
                	else if (cap_phase == CAP_PHASE_1) 
                	{
                		...
                		printk(KERN_INFO "CapProbe phase 1");
                	} 
                	else if (cap_phase == CAP_PHASE_2) 
                	{
                		printk(KERN_INFO "CapProbe phase 2");
                        ...
                        if (diff_c < 50) 			//cs218, check if error, maintaining precision
                        {
                        	del_timer(&tl);
                        	cap_size = 0;
                        	printk(KERN_INFO "CapProbe Finished! Total time = %ld.%06ld sec C = %ld.%03ld\n",
                        		total_time/1000000, total_time % 1000000,
                        		((long)(avg_c*10))/10, (long)(avg_c*1000)%1000);
                        } 
                        else 
                        {
                        	printk(KERN_INFO "... Restart CapProbe ...\n");
                        	if (cap_run%2==0) 
                        	{
                                // adjust packet size
                        		if (cap_var>0) 
                    ...
                    else if (cap_phase==CAP_PHASE_3) 
                    {
                    	...
                    	printk(KERN_INFO "CapProbe phase 3");
                    } 
                    else 
                    {
                    	printk(KERN_INFO "CAP_PHASE error!\n");
                    }
                ...
                else 
                {
                	...
                	{
                		printk(KERN_INFO "CapProbe serial num < sample MAX");
            ...
            {
            	printk(KERN_INFO "CapProbe not converged!\n");
            }
        }
    }
}
